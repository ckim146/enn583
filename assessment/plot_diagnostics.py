...
from vo_failure_tools import (
    ...
    show_frame_pair,
    blur_profile,
    plot_inliers_and_error_vs_frame,
)
...
def check_visual_odometry(dataset, full_dataset) -> bool:
    ...
    if len(student_poses) >= 2:
        ...
        frame_gt_rotation_deg = []        # true rotation per frame pair
        ...
        # Median Values
        relative_translation_rmse_med = math.sqrt(
            statistics.median(relative_translation_squared_errors)
        )
        relative_rotation_rmse_rad_med = math.sqrt(
            statistics.median(relative_rotation_squared_errors)
        )
        relative_rotation_rmse_deg_med = math.degrees(relative_rotation_rmse_rad_med)
        ...
        print("[INFO] Median frame-to-frame relative pose error:")
        print(f"       Translation RMSE: {relative_translation_rmse_med:.6f} m")
        print(
            f"       Rotation RMSE: {relative_rotation_rmse_rad_med:.6f} rad "
        ...
        # Heading error vs. how much the car actually turned
        ...
        # Trajectory colored by PnP inlier count (low = dark/red end of the colormap)
        n_pairs = len(student_poses) - 1
        if len(diagnostics) >= n_pairs:
            n_inliers = np.array([int(r["n_inliers"]) for r in diagnostics[:n_pairs]])
            plot_path = plot_trajectory_colored(
                student_poses,
                ground_truth_poses_relative_to_frame_zero,
                n_inliers,
                rejected_frames,
                err_label="PnP inliers per frame pair",
                save_path="results_vo_trajectory_inliers.png",
                title="Estimated trajectory colored by PnP inlier count",
                cmap="RdYlGn",                              # red = few inliers
                vmin=float(np.percentile(n_inliers, 1)),
                vmax=float(np.percentile(n_inliers, 99)),
                line_label="Estimate (colored by inliers)",
            )
            print(f"[INFO] Wrote inlier-colored trajectory to {plot_path}.")
            ts_path, r = plot_inliers_and_error_vs_frame(n_inliers, heading)
            print(f"[INFO] Wrote inliers vs. heading error to {ts_path} "
                  f"(Pearson r = {r:.2f}; negative = fewer inliers, larger error).")
        else:
            print(f"[INFO] Skipped inlier plots ({DIAGNOSTICS_CSV} missing or incomplete).")
